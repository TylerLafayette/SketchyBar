...
bool background_set_height(struct background *background, uint32_t height) {
  if (background->bounds.size.height == height)
    return false;
...
static bool background_set_border_color(struct background *background,
                                        uint32_t color) {
  return color_set_hex(&background->border_color, color);
}

static bool background_set_border_width(struct background *background,
                                        uint32_t border_width) {
  if (background->border_width == border_width)
    return false;
  background->border_width = border_width;
  return true;
}

static bool background_set_corner_radius(struct background *background,
                                         uint32_t corner_radius) {
  if (background->corner_radius == corner_radius)
    return false;
...
static bool
background_set_inverted_corner_radius(struct background *background,
                                      uint32_t inverted_corner_radius) {
  if (background->inverted_corner_radius == inverted_corner_radius)
    return false;
  background->inverted_corner_radius = inverted_corner_radius;
  return true;
}

static bool background_set_yoffset(struct background *background, int offset) {
  if (background->y_offset == offset)
    return false;
...
bool background_set_padding_left(struct background *background, uint32_t pad) {
  if (background->padding_left == pad)
    return false;
...
bool background_set_padding_right(struct background *background, uint32_t pad) {
  if (background->padding_right == pad)
    return false;
  background->padding_right = pad;
  return true;
}

bool background_clip_needs_update(struct background *background,
                                  struct bar *bar) {
  if (background->clip == 0.f || !background->enabled)
    return false;
  struct background *clip = background_get_clip(background, bar->adid);
  if (!CGRectEqualToRect(background->bounds, clip->bounds))
    return true;
  if (background->corner_radius != clip->corner_radius)
    return true;
  if (background->y_offset != clip->y_offset)
    return true;
  return false;
}

static void background_update_clip(struct background *background,
                                   struct background *clip) {
  memcpy(clip, background, sizeof(struct background));
  background_clear_pointers(clip);
}

struct background *background_get_clip(struct background *background,
                                       uint32_t adid) {
  if (adid < 1)
    return NULL;
  if (background->num_clips < adid) {
    background->clips = (struct background **)realloc(
        background->clips, sizeof(struct background *) * adid);
    memset(background->clips + background->num_clips, 0,
...
void background_clip_bar(struct background *background, int offset,
                         struct bar *bar) {
  if (!background_clips_bar(background))
    return;
  ...
  clip_rect(bar->window.context, background_bounds, background->clip,
            background->corner_radius);
}

void background_calculate_bounds(struct background *background, uint32_t x,
...
static void draw_rect(CGContextRef context, CGRect region,
                      struct color *fill_color, uint32_t corner_radius,
                      uint32_t line_width, struct color *stroke_color) {
  CGContextSetLineWidth(context, line_width);
  if (stroke_color)
    CGContextSetRGBStrokeColor(context, stroke_color->r, stroke_color->g,
                               stroke_color->b, stroke_color->a);
  CGContextSetRGBFillColor(context, fill_color->r, fill_color->g, fill_color->b,
                           fill_color->a);

  CGMutablePathRef path = CGPathCreateMutable();
  CGRect inset_region =
      CGRectInset(region, (float)(line_width) / 2.f, (float)(line_width) / 2.f);
...
void draw_inverted_rounded_rect_top(CGContextRef context, CGRect region,
  ...
  printf("called with region at %f, %f\n", region.origin.x,
         region.origin.y + region.size.height);

  CGMutablePathRef path = CGPathCreateMutable();

  CGFloat x1 = region.origin.x;
  CGFloat y1 = region.origin.y;

  CGPathMoveToPoint(path, NULL, x1, y1);
  CGPathAddLineToPoint(path, NULL, x1, y1 + corner_radius);
  CGPathAddLineToPoint(path, NULL, x1 + corner_radius, y1 + corner_radius);
  CGPathAddArcToPoint(path, NULL, x1, y1 + corner_radius, x1, y1,
                      corner_radius);
  // CGPathAddArcToPoint(path, NULL, x1 + corner_radius, y1, x1,
  //                     y1 + corner_radius, corner_radius);

  CGContextAddPath(context, path);
  CGContextDrawPath(context, kCGPathFillStroke);

  CGAffineTransform x_invert = CGAffineTransformMakeScale(-1.0, 1.0);
  CGMutablePathRef inverted_path =
      CGPathCreateMutableCopyByTransformingPath(path, &x_invert);

  CGAffineTransform shift =
      CGAffineTransformMakeTranslation(region.size.width, 0);
  CGMutablePathRef shifted_path =
      CGPathCreateMutableCopyByTransformingPath(inverted_path, &shift);

  CGRect bounding = CGPathGetBoundingBox(shifted_path);

  CGContextAddPath(context, shifted_path);
  CGContextDrawPath(context, kCGPathFillStroke);

  CFRelease(shifted_path);
  CFRelease(inverted_path);
  CFRelease(path);
}
...
void background_draw(struct background *background, CGContextRef context) {
  ...
  if ((background->border_color.a == 0 || background->border_width == 0) &&
      (background->color.a == 0) && !background->shadow.enabled &&
  ...
  if (background->inverted_corner_radius > 0) {
    background_bounds.origin.y += background->inverted_corner_radius;
    background_bounds.size.height -= background->inverted_corner_radius;

    struct CGRect inverted_corners_bounds = {
        {0, 0},
        {background_bounds.size.width, background->inverted_corner_radius}};
    draw_inverted_rounded_rect_top(context, inverted_corners_bounds,
                                   &background->color,
                                   background->inverted_corner_radius);
  }

  if (background->shadow.enabled) {
    CGRect bounds = shadow_get_bounds(&background->shadow, background_bounds);
    draw_rect(context, bounds, &background->shadow.color,
              background->corner_radius, background->border_width,
              &background->shadow.color);
  }

  draw_rect(context, background_bounds, &background->color,
            background->corner_radius, background->border_width,
...
void background_serialize(struct background *background, char *indent,
                          FILE *rsp, bool detailed) {
  fprintf(rsp,
          "%s\"drawing\": \"%s\",\n"
          "%s\"color\": \"0x%x\",\n"
          "%s\"border_color\": \"0x%x\",\n"
          ...
          "%s\"y_offset\": %d,\n"
          "%s\"clip\": %f,\n",
          indent, format_bool(background->enabled), indent,
          background->color.hex, indent, background->border_color.hex, indent,
          background->border_width, indent,
          background->overrides_height ? (int)background->bounds.size.height
                                       : 0,
          indent, background->corner_radius, indent, background->padding_left,
          indent, background->padding_right, indent, background->y_offset,
...
bool background_parse_sub_domain(struct background *background, FILE *rsp,
                                 struct token property, char *message) {
  bool needs_refresh = false;
  if (token_equals(property, PROPERTY_DRAWING))
    return background_set_enabled(
        background,
        evaluate_boolean_state(get_token(&message), background->enabled));
  else if (token_equals(property, PROPERTY_CLIP)) {
    struct token token = get_token(&message);
    ANIMATE_FLOAT(background_set_clip, background, background->clip,
    ...
    ANIMATE(background_set_corner_radius, background, background->corner_radius,
            token_to_int(token));
  } else if (token_equals(property, PROPERTY_INVERTED_CORNER_RADIUS)) {
    struct token token = get_token(&message);
    ANIMATE(background_set_inverted_corner_radius, background,
            background->inverted_corner_radius, token_to_int(token));
    ...
    ANIMATE_BYTES(background_set_border_color, background,
                  background->border_color.hex, token_to_int(token));
    ...
    if (key_value_pair.key && key_value_pair.value) {
      ...
      if (token_equals(subdom, SUB_DOMAIN_SHADOW))
        return shadow_parse_sub_domain(&background->shadow, rsp, entry,
                                       message);
        ...
        return color_parse_sub_domain(&background->border_color, rsp, entry,
                                      message);
      } else {
