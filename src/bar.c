...
bool bar_draws_item(struct bar *bar, struct bar_item *bar_item) {
  if (!bar_item->drawing || !bar->shown || bar->hidden)
    return false;
...
static void bar_calculate_popup_anchor_for_bar_item(struct bar *bar,
                                                    struct bar_item *bar_item) {
  if (bar->adid != g_bar_manager.active_adid)
    return;
...
void bar_order_item_windows(struct bar *bar) {
  if (bar->sid < 1 || bar->adid < 1 || !bar->shown)
    return;
  ...
  if (!g_bar_manager.bar_needs_update) {
    for (int i = 0; i < g_bar_manager.bar_item_count; i++) {
      ...
      g_bar_manager.bar_needs_update |=
          window->needs_move || window->needs_resize ||
...
void bar_draw(struct bar *bar, bool forced) {
  ...
  for (int i = 0; i < g_bar_manager.bar_item_count; i++) {
    ...
    if (g_bar_manager.bar_needs_update) {
      bar_item_clip_bar(bar_item, window->origin.x - bar->window.origin.x, bar);
    }
...
static void bar_calculate_bounds_top_bottom(struct bar *bar) {
  ...
  uint32_t bar_left_first_item_x =
      max(g_bar_manager.background.padding_left, 0);

  uint32_t bar_right_first_item_x =
      bar->window.frame.size.width -
      max(g_bar_manager.background.padding_right, 0);

  uint32_t bar_center_first_item_x =
      (bar->window.frame.size.width - center_length) / 2 - 1;

  uint32_t bar_center_right_first_item_x =
      (bar->window.frame.size.width + notch_width) / 2 - 1;

  uint32_t bar_center_left_first_item_x =
      (bar->window.frame.size.width - notch_width) / 2 - 1;
  ...
  for (int i = 0; i < g_bar_manager.bar_item_count; i++) {
    ...
    if (bar_item->position == POSITION_RIGHT ||
      ...
      *next_position =
          min(*next_position - bar_item_display_length -
                  bar_item->background.padding_right,
              bar->window.frame.size.width - bar_item_display_length);
    } else {
      *next_position +=
          max((int)-*next_position, bar_item->background.padding_left);
    }
    ...
    if (bar_item->position == POSITION_RIGHT ||
      ...
      *next_position +=
          bar_item->has_const_width
              ...
              : (bar_item_length + bar_item->background.padding_right);
    }
  }
  ...
  for (int i = 0; i < g_bar_manager.bar_item_count; i++) {
    ...
    window_set_frame(
        bar_item_get_window(bar_item->group->members[0], bar->adid),
...
static void bar_calculate_bounds_left_right(struct bar *bar) {
  ...
  uint32_t bar_left_first_item_y =
      max(g_bar_manager.background.padding_left, 0);

  uint32_t bar_right_first_item_y =
      bar->window.frame.size.height -
      max(g_bar_manager.background.padding_right, 0);

  uint32_t bar_center_first_item_y =
      (bar->window.frame.size.height - 2 * g_bar_manager.margin -
       center_length) /
          2 -
      1;

  uint32_t bar_center_right_first_item_y =
      (bar->window.frame.size.height + notch_width) / 2 - 1;

  uint32_t bar_center_left_first_item_y =
      (bar->window.frame.size.height - notch_width) / 2 - 1;
  ...
  for (int i = 0; i < g_bar_manager.bar_item_count; i++) {
    ...
    if (bar_item->position == POSITION_RIGHT ||
      ...
      *next_position =
          min(*next_position - bar_item_display_height -
                  bar_item->background.padding_right,
              bar->window.frame.size.height - bar_item_display_height);
    } else {
      *next_position +=
          max((int)-*next_position, bar_item->background.padding_left);
    }
    ...
    CGPoint shadow_offsets = bar_item_calculate_shadow_offsets(bar_item);
    bar_item_calculate_bounds(bar_item, bar_item_display_height,
    ...
    CGRect frame = {
        {bar->window.origin.x + x - max(shadow_offsets.x, 0),
         bar->window.origin.y + *next_position + -max(-bar_item->y_offset, 0)},
        {g_bar_manager.background.bounds.size.height,
    ...
    if (bar_item->position == POSITION_RIGHT ||
      ...
      *next_position +=
          bar_item->has_const_width
              ? bar_item->custom_width - bar_item->background.padding_left
              : (bar_item_display_height + bar_item->background.padding_right);
    }
  }
}
...
static CGRect bar_get_frame(struct bar *bar) {
  ...
  if (g_bar_manager.position == POSITION_LEFT ||
    ...
    return (CGRect){
        {origin.x, origin.y},
        {g_bar_manager.background.bounds.size.height, bounds.size.height}};
  } else {
    ...
    return (CGRect){
        {origin.x, origin.y},
        {bounds.size.width, g_bar_manager.background.bounds.size.height}};
  }
}
